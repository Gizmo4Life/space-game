...
- **Rule:** Drift amount is stochastic but biased by the performance delta (default $0.05$ per drift).
- **Rule:** A drift event may trigger a **Ship Redesign** to apply new DNA to future hulls.
- **Rule:** Telemetry: All drift events are recorded via `game.faction.dna.drift` spans with `axis`, `old_value`, and `new_value` attributes.
...
- DNA drift events are logged and traceable in the `EconomyManager` update cycle via `game.faction.dna.drift` spans.
