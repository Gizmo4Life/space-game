...
- **Action:** Execute [Standard Definition](standard-definition.md) to select/assign appropriate patterns. Specifically, ensure future components use [Interface Segregation](/docs/developer/pattern/cpp-interface-segregation.md) for dependencies and [Type-Safe Handles](/docs/developer/pattern/cpp-type-safe-handles.md) for domain IDs.
...
- **Action:** Write the physical code in `/src` adhering strictly to the selected Patterns. Isolate any external library usage using [External API Facade](/docs/developer/pattern/cpp-external-api-facade.md).
